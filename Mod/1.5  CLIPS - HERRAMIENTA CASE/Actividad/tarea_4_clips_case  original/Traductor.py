...
def process_xmi_file(input_file, output_file):

    try:
        with open(input_file, 'r') as archivo:
            xmi_data = archivo.read()

        root = parse_xmi(input_file)
        ...
        write_clips_file(clips_facts, output_file)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Uso: python Traductor.py <archivo_xmi> <archivo_clips>")
    else:
...
(deffunction call-generate-methods (?class-name ?operations)
   (do-for-all-facts ((?op operation))
      ...
      (printout t "   " ?visibility " " ?type " " ?name "()" " {" crlf
        ...
        # Agregar las reglas después de las funciones
        file.write('''
